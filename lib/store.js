...
const merchants = {
  ...
  async insert(m) {
    await pool.query(
      'INSERT INTO merchants (id,email,public_key,secret_key,data) VALUES ($1,$2,$3,$4,$5::jsonb)',
      [m.id, m.email, m.publicKey, m.secretKey, JSON.stringify(m)],
    );
    return m;
  },
  async update(m) {
    await pool.query(
      'UPDATE merchants SET data=$1::jsonb, email=$2, public_key=$3, secret_key=$4 WHERE id=$5',
      [JSON.stringify(m), m.email, m.publicKey, m.secretKey, m.id],
    );
    return m;
  },
};
...
module.exports = { load, persist, merchants, charges, events, payouts, settlements, verifications };
