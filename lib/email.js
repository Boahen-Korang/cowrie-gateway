...
async function sendEmail({ to, subject, html, text }) {
  ...
  if (gmailUser && gmailPass) {
    ...
    await t.sendMail({ from: `"Cowrie Payments" <${gmailUser}>`, to: recipients.join(', '), subject, html, text });
    return;
  }
  ...
  <tr><td align="center">
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
      ...
      <tr><td style="padding:36px 32px 28px">
        ...
        <p style="margin:0 0 24px;font-size:32px;font-weight:800;color:#1a0a2e;letter-spacing:-.5px">${formatted}</p>
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8f5ff;border-radius:12px;padding:20px;margin-bottom:24px">
          <tr><td style="padding:6px 0;font-size:13px;color:#888;width:130px">Merchant</td><td style="padding:6px 0;font-size:14px;color:#1a0a2e;font-weight:600">${merch}</td></tr>
          <tr><td style="padding:6px 0;font-size:13px;color:#888">Customer</td><td style="padding:6px 0;font-size:13px;color:#1a0a2e">${custEmail}</td></tr>
          ${payer ? `<tr><td style="padding:6px 0;font-size:13px;color:#888">Name</td><td style="padding:6px 0;font-size:13px;color:#1a0a2e">${payer}</td></tr>` : ''}
          <tr><td style="padding:6px 0;font-size:13px;color:#888">Method</td><td style="padding:6px 0;font-size:13px;color:#1a0a2e">${escHtml(methodLabel)}</td></tr>
          <tr><td style="padding:6px 0;font-size:13px;color:#888">Reference</td><td style="padding:6px 0;font-size:12px;color:#1a0a2e;font-family:'Courier New',monospace">${ref}</td></tr>
        </table>
...
module.exports = { sendOtp, sendKycApproved, sendKycRejected, sendPendingTransferAlert, sendDepositAlert };
